...
t2 <- ggplot(pca_complete2, aes(x = Axis1, y = Axis2)) +
  ...
  #stat_ellipse(aes(x = Axis1, y = Axis2, group = cluster, color = cluster), level = 0.95, linetype = 2, size = 1, alpha = 0.8) +
  geom_text_repel(aes(label = new_id, color = color_by_stage), 
                  size = 3, max.overlaps = 38, point.padding = 0.5, box.padding = 0.5, alpha = 1) +
  scale_fill_identity() +
  #scale_color_manual(values = c("1" = "dodgerblue", "2" = "salmon", "3" = "grey60", "indianred3" = "indianred3", "black" = "black")) +
